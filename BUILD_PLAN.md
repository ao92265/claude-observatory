...
CCPilot depends on HookScope telemetry. Build HookScope first, harvest data, then bolt CCPilot on top.
...
│   ├── hookscope/         live timeline TUI + web UI
│   ├── ccpilot/           recommendation + A/B engine
...
### Phase 1 — HookScope MVP (week 1-2)
...
- [ ] CLI: `observatory hookscope` (start daemon + TUI), `observatory hookscope --install` (writes settings.json shim)
...
### Phase 2 — HookScope web UI (week 3)
...
### Phase 3 — CCPilot recommender (week 4-6)
...
- [ ] `observatory ccpilot ab <suggestion-id>` runs and compares
...
2. `pytest -q` green
3. Coverage ≥ 80% on `core/` and `ccpilot/recommender/`
...
- [ ] Two demo screencasts (HookScope live, CCPilot suggest+AB)
...
| Risk | Mitigation |
|---|---|
| Anthropic ships native observability and obsoletes HookScope | Move fast (6-week window). Focus on plugin-author niche they're unlikely to serve. |
...
| CCPilot suggests harmful CLAUDE.md edits | Default dry-run. Diff preview. A/B evidence required for `--apply`. |
...
- HookScope: install in < 30s, captures every hook on macOS + Linux, ships with 5 built-in views
- CCPilot: produces ≥ 1 high-confidence recommendation on any user with ≥ 50 sessions; A/B harness runs end-to-end on synthetic task
